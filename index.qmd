...
#| output-location: slide
#| out-width: "100%"
#| label: fig-conditional-kde-beta-ci-2
...
f.hrl <- npcdens(txdat=x,exdat=rep(1987,100),tydat=y,eydat=seq(min(y),100,length=100))
...
par(mfrow=c(2,2),cex=.75,mar=c(4,4,4,2))
...
lines(seq(min(y),100,length=100),fitted(f.hrl),col=2,lty=2,lwd=2)
lines(seq(min(y),100,length=100),predict(f.bkcde,newdata=data.frame(x=rep(1987,100),y=seq(min(y),100,length=100))),col=3,lty=3,lwd=3)
legend("topright",legend=c("density() classical kernel","HRL classical kernel","BKPA boundary kernel"),col=1:3,lty=1:3,lwd=c(1,2,3),bty="n")
plot(f.bkcde,persp=FALSE,x.eval=1987,y.eval=seq(min(y),100,length=100),xlim=c(0,100))
plot(f.bkcde,persp=FALSE,x.eval=1987,y.eval=seq(min(y),100,length=100),xlim=c(0,100),ci=TRUE,ci.method="Simultaneous",ci.preplot=FALSE,B=1000)
plot(f.bkcde,x.eval=seq(min(x),max(x),length=25),y.eval=seq(min(y),100,length=25),proper=FALSE,ci=TRUE,ci.method="Simultaneous",ci.preplot=FALSE,B=1000)
```
:::
...
-   We compare the performance of the proposed method using empirical bounds with the methods of @HALL_RACINE_LI:2004 and @fyt:1996, which use a classical kernel presuming infinite support (inappropriate for the Beta).
    ...
    kable(rmse.comp.beta.ml.irr, col.names=c("n","BCPA","HLR","FYT"), digits=2,align='r')    
    ```
    ...
    kable(rmse.comp.beta.ls.irr, col.names=c("n","BCPA","HLR","FYT"), digits=2,align='r')    
    ```    
    ...
    kable(rmse.comp.beta.ml.quad, col.names=c("n","BCPA","HLR","FYT"), digits=2,align='r')
    ```
    ...
    kable(rmse.comp.beta.ls.quad, col.names=c("n","BCPA","HLR","FYT"), digits=2,align='r')
    ```
    :::
...
-   We compare the performance of the proposed method using empirical bounds with the methods of @HALL_RACINE_LI:2004 and @fyt:1996, which use a classical kernel presuming infinite support (appropriate for the Gaussian).
    ...
    #| tbl-cap: Median Relative RMSE Irrelevant ML
    kable(rmse.comp.norm.ml.irr, col.names=c("n","BCPA","HLR","FYT"), digits=2,align='r')    
    ```
    ...
    #| tbl-cap: Median Relative RMSE Irrelevant LS
    kable(rmse.comp.norm.ls.irr, col.names=c("n","BCPA","HLR","FYT"), digits=2,align='r')    
    ```    
    ...
    #| tbl-cap: Median Relative RMSE Quadratic ML
    kable(rmse.comp.norm.ml.quad, col.names=c("n","BCPA","HLR","FYT"), digits=2,align='r')
    ```
    ...
    #| tbl-cap: Median Relative RMSE Quadratic LS
    kable(rmse.comp.norm.ls.quad, col.names=c("n","BCPA","HLR","FYT"), digits=2,align='r')
    ```
    :::
