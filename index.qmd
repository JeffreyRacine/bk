...
format:
  revealjs:
    ...
    footer: "Boundary Corrected Adaptive CKDE | <https://jeffreyracine.github.io/bk> |&copy; Jeffrey S. Racine"
...
-   You are invited to participate in this "interactive seminar". Hopefully you came with your laptop, the Firefox browser, R/RStudio, and a the suggested libraries installed.

-   This talk is about a proposed boundary adaptive, polynomial order adaptive, conditional kernel density estimator. The talk is divided into the following sections:
...
-   Nonparametric (local) kernel-based density estimation has a rich history [@ROSENBLATT:1956; @PARZEN:1962]: \begin{equation*}
    \hat f(x)=\frac{1}{n}\sum_{i=1}^n \frac{1}{h}K\left(\frac{x-X_i}{h}\right)
    \end{equation*}

-   Conditions for consistency are well-established and typically require that the bandwidth vanishes asymptotically (i.e., that $h\to 0$ as $n\to\infty$)

-   However, in the presence of finite bounds (e.g., $X\in[a,b]$, $a<b$, $|a|<\infty$, $|b|<\infty$) commonly used kernels (e.g., Gaussian, Epanechnikov) are known to behave poorly at the boundary of support (i.e., may converge very slowly at or near boundaries)
...
-   Consider the range of boundary kernel functions existing in the literature:

-   Beta ($x\in[a,b]$, $|a|<\infty$, $|b|<\infty$, $a<b$)

    -   @CHEN:1999
    -   @BOUEZMARNI_ROLIN:2003
    -   @ZHANG_KARUNAMUNI:2010
    -   @IGARASHI:2016
    -   @RACINE_LI_WANG:2024

-   Gamma ($x\in[a,\infty]$, $|a|<\infty$)

    -   @CHEN:2000
    -   @MALEC201457

-   Inverse and Reciprocal Inverse Gaussian ($x\in[a,b]$, $|a|<\infty$, $|b|<\infty$, $a<b$)

    -   @SCAILLET:2004
    -   @IGARASHI_KAKIZAWA:2014

## KDE with Bounded Data - A Priori Information

-   The use of boundary kernels is not without its issues. @SCOTT:1992, Page 149, writes
...
-   Another problem is that virtually all existing boundary kernels require the bounds to be known a priori

-   This is because they proceed from weight functions that are themselves bounded on $[a,b]$, e.g., the Beta ($x\in[a,b]$) and Gamma kernels ($x\in[a,\infty]$) where $a$ and $b$ are finite
...
-   But in the presence of *unknown* bounds, @RACINE_LI_WANG:2024 considered a simple kernel function that can *automatically* adapt *and* nests classical kernels where $Y\in[-\infty,\infty]$ as a special case
...
-   The conditional density $f(y|x)=f(y,x)/f(x)$ is a fundamental object in statistics

-   Related objects include the conditional distribution function $F(y|x)=\int f(y|x)/f(x)=\int f(y,x)dy/f(x)$, the conditional expectation $E(Y|X=x)=\int yf(y|x)dy$, and the conditional quantile function $Q(p|x)=\inf\{y:F(y|x)\ge p\}$

-   Also, derivatives are often required for constructing, say, "marginal effects" in econometrics such as $d E(Y|X=x)/d x$ along with counterparts for the conditional distribution function and quantile function

-   Consistent estimation, however, must be based on nonparametric estimation of the conditional density $f(y|x)$
...
-   Another problem [@FAN_GIJBELS:1995, p 59; @FAN_GIJBELS:1996] is the conventional wisdom about polynomial order selection:
...
#| fig-cap: Conditional KDE for $N(0,1)$ with empirical bounds
library(bkcde)
set.seed(42)
n <- 250
a <- 0
