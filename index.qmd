...
format:
  revealjs:
    ...
    multiplex:
      ...
      client-url: "https://multiplex.up.railway.app"    
    preview-links: auto
...
sample_dirs <- function(root) {
  dirs <- list.dirs(root, recursive = FALSE, full.names = TRUE)
  dirs <- dirs[grepl("^n_[0-9]+$", basename(dirs))]
  dirs[order(as.integer(sub("^n_", "", basename(dirs))))]
}

read_sample_matrix <- function(root, row_builder) {
  dirs <- sample_dirs(root)
  stopifnot(length(dirs) > 0L)
  do.call(rbind, lapply(dirs, row_builder))
}

read_candidate_summary <- function(root) {
  median.dat <- read_sample_matrix(root, function(dir) {
    c(scan(file.path(dir, "n.dat"), quiet = TRUE),
      scan(file.path(dir, "median_rmse_candidates.out"), quiet = TRUE),
      scan(file.path(dir, "median_rmse_models.out"), quiet = TRUE)[1])
  })

  cbind(median.dat[, 1],
        formatC(
          sweep(
            median.dat[, 2:ncol(median.dat), drop = FALSE],
            1,
            median.dat[, ncol(median.dat), drop = FALSE],
            FUN = "/"
          ),
          format = "f",
          digits = 2
...
read_scale_summary <- function(root, stem) {
  read_sample_matrix(root, function(dir) {
    c(scan(file.path(dir, "n.dat"), quiet = TRUE),
      apply(read.table(file.path(dir, paste0(stem, ".out")), header = FALSE), 2, median),
      apply(read.table(file.path(dir, paste0(stem, "_hrl.out")), header = FALSE), 2, median),
      apply(read.table(file.path(dir, paste0(stem, "_fyt.out")), header = FALSE), 2, median))
  })
}

read_model_comparison <- function(root) {
  median.dat <- read_sample_matrix(root, function(dir) {
    c(scan(file.path(dir, "n.dat"), quiet = TRUE),
      scan(file.path(dir, "median_rmse_models.out"), quiet = TRUE))
  })

  cbind(median.dat[, 1],
        formatC(
          sweep(
            median.dat[, 2:4, drop = FALSE],
            1,
            median.dat[, 2, drop = FALSE],
            FUN = "/"
          ),
          format = "f",
          digits = 2
...
candidate_col_names <- function(dat) {
  c("n", paste0("p=", 0:(ncol(dat) - 3L)), "BKPA")
}

style_candidate_table <- function(dat, highlight_p) {
  kable(dat, col.names = candidate_col_names(dat), digits = 2, align = "r") %>%
    row_spec(nrow(dat), bold = TRUE, color = "lightgreen") %>%
    column_spec(c(highlight_p + 2L, ncol(dat)), bold = TRUE, color = "green")
}
```
...
Let's consider an illustration where we estimate the age distribution for working age males from a random sample taken from the Current Population Survey (CPS) using the classical kernel and that of @RACINE_LI_WANG:2024 when bounds are *not* assumed to be known *a priori* (we use the empirical support $[\min(X),\max(X)]$)
...
-   The local polynomial extension of this estimator [@fyt:1996] is given by \begin{equation*}
...
#| echo: false
#| cache: false
rmse.beta.ml.irr <- read_candidate_summary("../work_irr_empirical_bounds/3D_cvml/beta")
rmse.beta.ml.quad <- read_candidate_summary("../work_quad_empirical_bounds/3D_cvml/beta")
rmse.beta.ls.irr <- read_candidate_summary("../work_irr_empirical_bounds/3D_cvls/beta")
rmse.beta.ls.quad <- read_candidate_summary("../work_quad_empirical_bounds/3D_cvls/beta")
rmse.norm.ml.irr <- read_candidate_summary("../work_irr_empirical_bounds/3D_cvml/norm")
rmse.norm.ml.quad <- read_candidate_summary("../work_quad_empirical_bounds/3D_cvml/norm")
rmse.norm.ls.irr <- read_candidate_summary("../work_irr_empirical_bounds/3D_cvls/norm")
rmse.norm.ls.quad <- read_candidate_summary("../work_quad_empirical_bounds/3D_cvls/norm")
```
...
-   If you knew *ex ante* which polynomial order to select, you would use that
    ...
    style_candidate_table(rmse.beta.ml.irr, highlight_p = 0)
    ```
    ...
    style_candidate_table(rmse.beta.ls.irr, highlight_p = 0)
    ```
    ...
    style_candidate_table(rmse.beta.ml.quad, highlight_p = 2)
    ```
    ...
    style_candidate_table(rmse.beta.ls.quad, highlight_p = 2)
    ```
    :::
...
-   If you knew *ex ante* which polynomial order to select, you would use that
    ...
    #| tbl-cap: Relative RMSE Irrelevant ML
    style_candidate_table(rmse.norm.ml.irr, highlight_p = 0)
    ```
    ...
    #| tbl-cap: Relative RMSE Irrelevant LS
    style_candidate_table(rmse.norm.ls.irr, highlight_p = 0)
    ```
    ...
    #| tbl-cap: Relative RMSE Quadratic ML
    style_candidate_table(rmse.norm.ml.quad, highlight_p = 2)
    ```
    ...
    #| tbl-cap: Relative RMSE Quadratic LS
    style_candidate_table(rmse.norm.ls.quad, highlight_p = 2)
    ```
    :::
...
## LS-CV summaries
## norm
h.norm.cvls.irr <- read_scale_summary("../work_irr_empirical_bounds/3D_cvls/norm", "h")
sf.norm.cvls.irr <- read_scale_summary("../work_irr_empirical_bounds/3D_cvls/norm", "sf")
h.norm.cvls.quad <- read_scale_summary("../work_quad_empirical_bounds/3D_cvls/norm", "h")
sf.norm.cvls.quad <- read_scale_summary("../work_quad_empirical_bounds/3D_cvls/norm", "sf")

## beta
h.beta.cvls.irr <- read_scale_summary("../work_irr_empirical_bounds/3D_cvls/beta", "h")
sf.beta.cvls.irr <- read_scale_summary("../work_irr_empirical_bounds/3D_cvls/beta", "sf")
h.beta.cvls.quad <- read_scale_summary("../work_quad_empirical_bounds/3D_cvls/beta", "h")
sf.beta.cvls.quad <- read_scale_summary("../work_quad_empirical_bounds/3D_cvls/beta", "sf")
```
...
## ML-CV summaries 
## norm
h.norm.cvml.irr <- read_scale_summary("../work_irr_empirical_bounds/3D_cvml/norm", "h")
sf.norm.cvml.irr <- read_scale_summary("../work_irr_empirical_bounds/3D_cvml/norm", "sf")
h.norm.cvml.quad <- read_scale_summary("../work_quad_empirical_bounds/3D_cvml/norm", "h")
sf.norm.cvml.quad <- read_scale_summary("../work_quad_empirical_bounds/3D_cvml/norm", "sf")

## beta
h.beta.cvml.irr <- read_scale_summary("../work_irr_empirical_bounds/3D_cvml/beta", "h")
sf.beta.cvml.irr <- read_scale_summary("../work_irr_empirical_bounds/3D_cvml/beta", "sf")
h.beta.cvml.quad <- read_scale_summary("../work_quad_empirical_bounds/3D_cvml/beta", "h")
sf.beta.cvml.quad <- read_scale_summary("../work_quad_empirical_bounds/3D_cvml/beta", "sf")
```
...
#| cache: false
## HRL/FYT comparison
rmse.comp.beta.ml.irr <- read_model_comparison("../work_irr_empirical_bounds/3D_cvml/beta")
rmse.comp.beta.ml.quad <- read_model_comparison("../work_quad_empirical_bounds/3D_cvml/beta")
rmse.comp.beta.ls.irr <- read_model_comparison("../work_irr_empirical_bounds/3D_cvls/beta")
rmse.comp.beta.ls.quad <- read_model_comparison("../work_quad_empirical_bounds/3D_cvls/beta")
rmse.comp.norm.ml.irr <- read_model_comparison("../work_irr_empirical_bounds/3D_cvml/norm")
rmse.comp.norm.ml.quad <- read_model_comparison("../work_quad_empirical_bounds/3D_cvml/norm")
rmse.comp.norm.ls.irr <- read_model_comparison("../work_irr_empirical_bounds/3D_cvls/norm")
rmse.comp.norm.ls.quad <- read_model_comparison("../work_quad_empirical_bounds/3D_cvls/norm")
```
